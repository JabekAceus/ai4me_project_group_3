...
class BottleNeck(nn.Module):
    def __init__(self, in_dim, out_dim, projectionFactor, *, dropoutRate=0.01, dilation=1, asym: bool = False, use_se=False):
        super().__init__()
    ...
    def forward(self, in_):
        ...
        return self.PReLU_out(main + self.se(other))
...
class BottleNeckDownSampling(BottleNeck):
     ...
     def forward(self, in_):
        ...
        output = self.PReLU_out(main_downsampled + self.se(other))
...
class ENet(nn.Module):
    def __init__(self, in_dim: int, out_dim: int, use_se=False, use_attention=False, improved_upsampling=False, **kwargs):
        ...
        self.bottleneck1_0 = BottleNeckDownSampling(K, K * 4, 4, use_se=use_se)
        self.bottleneck1_1 = nn.Sequential(*[BottleNeck(K * 4, K * 4, 4, use_se=use_se) for _ in range(4)])
        self.bottleneck2_0 = BottleNeckDownSampling(K * 4, K * 8, 4, use_se=use_se)
        self.bottleneck2_1 = nn.Sequential(
            BottleNeck(K * 8, K * 8, 4, dropoutRate=0.1, use_se=use_se),
            BottleNeck(K * 8, K * 8, 4, dilation=2, use_se=use_se),
        )
        self.bottleneck3 = BottleNeck(K * 8, K * 4, 4, dropoutRate=0.1, use_se=use_se)
        self.bottleneck4_0 = BottleNeckUpSampling(K*4 + K*4, K*4, 4, use_se=use_se)
        self.bottleneck4_1 = BottleNeck(K*4, K, 4, use_se=use_se)
        self.bottleneck5_0 = BottleNeckUpSampling(K + K, K, 4, use_se=use_se)
        self.bottleneck5_1 = BottleNeck(K, K, 4, use_se=use_se)
...
class ENet2_5D(ENet):
    def __init__(self, in_dim: int, out_dim: int, **kwargs):
        K: int = kwargs.get("kernels", 16)
        super().__init__(in_dim=K, out_dim=out_dim, use_se=True, **kwargs)
